...
# Load packages
packages <- c("tidyverse", "openxlsx", "gridExtra", "caret","randomForest", "knitr")
...
**TIP\:** When using Rstudio

* Short cuts can be very helpful especially inserting a new chunk using `ctrl+alt+i`
* To run code highlight it and press `ctrl+Enter`
* To view a dataframe double click on it's name in the "Environment" tab
...
```{r using sapply, results='hide'}
#hiding results mean that the code is run , but not displayed in the knitr output. This can be useful in certain cases... this isn't one of them
...
```{r, results='hide'}
#In this chunk hiding the results is useful as needs to be modified, but I don't wan tot take up a lot of space in the final document with the summary command. I could put summary in a separate chunk which doesn't get evaluated, this would be a good idea for a computationally intense action, but in this case it doesn't matter
...
df %>% arrange(desc(mpg)) %>% group_by(cyl) %>% summarise_all(first) %>%
  kable( ., format="markdown", caption="Top 5 unordered")


```


```{r Exercise 1: Data Cleaning, eval=FALSE}
#3
...
#The reuslts are hidden as I don't want to diplay the results of the logical test `dir.create`
...
5. make a denisty plot using this data, how can this be useful?
...
When you are making plots, barplots should only be used if you are using raw counts across categorical values, if you are using mean or any other statistic you are better off using a boxplot. Boxplots contain much more information about data than barplots. If you want to know how to put error bars on a bar plot you're doing it wrong, use a box plot. 
If you are planning on making a 3D pie chart, do a different degree.

#Tidying Data

The tidyverse uses a concept called [tidy data](http://vita.had.co.nz/papers/tidy-data.pdf). Ggplot uses data in the long form of a tidy data set. in order to get data into this form the `gather` function is needed. Gather can be a bit tricky to work out so it's worth spending a bit of time understanding it. 

1. Assign the "WorldPhones" dataset to the "df" variable
2. Convert df from a matrix to a dataframe (Packages in the tidyverse only work with dataframes) and turn the row names in to a variable called year
...
df <-WorldPhones %>% as.data.frame() %>% mutate(Year = rownames(.)) %>%
 ...
 df %>% 
   ...
   geom_line() +
   labs(x="Year of Survey", y= "Number of Phones ('000s)", title="Phone Lines per Continent")

```

#Modelling

The bit that everyone body wants to get to in statistical programming is modelling. This part of the tutorial isn't supposed to teach you about various machine learning techniques but how to apply them in R. Having said that we will cover the two main branches of machine learning Supervised and Unsupervised

#Supervised learning
Supervised learning is broadly speaking machine learning when you know the outcome you are trying to predict. This could be the results of sports events, age of death, rate of growth, anythign with a measurable (directly or indirectly) result.

##Regression
Regression predicts a continuous variable for amount earned by a company next quarter. The simplest form of regression that is generally considered is multi-linear regression $(X'X)^{-1}X'y = \beta$, the big sister of the "plot  x against y and draw a line through the dots" everyone learnt about in school.
...
## Classification
When the dependent variable is in distinct categories, classification is used. Classic examples of this include SPAM vs HAM and Cat vs Dog. The previous examples are binary outcomes, however there can also be multiclass classification which is more fiddly mathmatically but makes very little difference when implementing in R.
...
#Unsupervised
Unsupervised learning doesn't predict an outcome it often looks for patterns in the data which imply a structure. Clustering algorithms such as Hierarchichal and k-means are good examples of unsupervised learning. 
